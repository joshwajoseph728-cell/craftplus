...
export const Sidebar: React.FC<SidebarProps> = ({ onOpenCreate }) => {
  ...
  const navItems = [
    ...
    { label: 'Explore Works', path: '/explore', icon: Compass },
  ...
  return (
    <aside className="hidden md:flex flex-col justify-between w-64 lg:w-72 h-screen sticky top-0 px-4 py-6 border-r border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-surface-dark transition-colors select-none">
      ...
      <div className="space-y-6">
        ...
        {/* Showcase Work Action */}
        <div className="px-2">
          <button
            onClick={onOpenCreate}
            className="w-full py-3 px-4 rounded-2xl bg-gradient-to-r from-brand-600 via-pink-600 to-accent-500 text-white font-bold text-sm shadow-md hover:shadow-glow-brand flex items-center justify-center gap-2.5 transition-all duration-200 hover:opacity-95 active:scale-[0.98]"
          >
        ...
        <nav className="space-y-1.5 px-2">
          {navItems.map(item => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  'flex items-center justify-between px-3.5 py-3 rounded-2xl font-semibold text-sm transition-all duration-200 group',
                  isActive
      ...
      <div className="space-y-3 px-2 pt-4 border-t border-slate-100 dark:border-slate-800/80">
        <NavLink
          to="/settings"
          className={({ isActive }) =>
            cn(
              'flex items-center gap-3.5 px-3.5 py-2.5 rounded-xl font-semibold text-xs transition-colors',
              isActive
