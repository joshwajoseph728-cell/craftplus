...
export const ChatWindow: React.FC<ChatWindowProps> = ({ recipient, onBack }) => {
  ...
  const handleSendVoiceNote = async () => {
    ...
    const tempMsg: Message = {
      ...
      content: `🎙️ Voice Note (${formattedDuration})`,
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    const content = inputText.trim() || (attachedImage ? '📷 Photo Attachment' : codeSnippet ? '💻 Code Snippet' : '');
  ...
  return (
    ...
    >
        ...
        {displayedMessages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 space-y-3">
            ...
            <div className="pt-2 flex items-center gap-2">
              <button
                onClick={() => setInputText(`Hey @${recipient.username}! 👋 Loved your projects on CraftPulse.`)}
      ...
      <form
        ...
        className="p-3 md:p-4 bg-white dark:bg-surface-cardDark border-t border-slate-200/80 dark:border-slate-800/80 flex items-center gap-2 z-20"
      >
        ...
        <button
          ...
          className="p-2.5 rounded-2xl text-slate-500 hover:text-brand-500 hover:bg-brand-500/10 transition-colors shrink-0"
        ...
        <button
          ...
          className="p-2.5 rounded-2xl text-slate-500 hover:text-amber-500 hover:bg-amber-500/10 transition-colors shrink-0"
          title="Insert Emoji"
        >
        ...
        <input
          ...
          placeholder={`Message ${recipient.username} (Press Enter to send, paste pictures with Ctrl+V)...`}
          className="flex-1 px-4 py-2.5 text-xs md:text-sm rounded-2xl bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-slate-100 border border-transparent focus:border-brand-500 outline-none transition-colors"
        />
          ...
          <div className="relative max-w-4xl max-h-[90vh] flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 flex items-center gap-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                download="craftpulse-attachment.jpg"
